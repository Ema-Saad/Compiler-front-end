...
class CodeGen:
    ...
    def add_initial(self):
        ...
        for feature in main_class.features:
            if isinstance(feature, eman.Attribute):
                data_type = ""
                value = ""
                flag = False
                if feature.type == "Int":
    ...
    def add_statement(self,stmt):
        if isinstance(stmt,eman.Assignment):
            self.add_statement(stmt.expr)
            self.add_left_value(stmt.ident)
        if isinstance(stmt,eman.BinaryOperation):
            self.add_binary_operation(stmt)
        if isinstance(stmt,eman.Ident):
            self.write("lw $a0 " + str(4 * 1) + "($fp)\n")
        if isinstance(stmt, eman.FunctionCall):
            tmp = ""
    ...
    def add_main_elements(self):
        ...
        for feature in self.main_class.features:
            if isinstance(feature, eman.Method) and feature.ident.name == 'main':
                print (feature)
                main_block = feature
                break
        ...
        for element in main_block.expr.elements:
            if isinstance(element, eman.FunctionCall):
